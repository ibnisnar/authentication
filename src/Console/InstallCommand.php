...
namespace Ibnisnar\Authentication\Console;
...
#[AsCommand(name: 'authentication:install')]
class InstallCommand extends Command implements PromptsForMissingInput
{
     ...
     * @var string
     */
    protected $signature = 'authentication:install {stack : The development stack that should be installed (livewire)}
     ...
     * @var string
     */
    protected $description = 'Install the Authentication components and resources';
    ...
    public function handle()
    {
        if (! in_array($this->argument('stack'), ['livewire'])) {
            $this->components->error('Invalid stack. Supported stacks are and [livewire].');

            return 1;
        }

        // Publish...
        $this->callSilent('vendor:publish', ['--tag' => 'authentication-config', '--force' => true]);
        $this->callSilent('vendor:publish', ['--tag' => 'authentication-migrations', '--force' => true]);
        ...
        // Configure API...
        if ($this->option('api')) {
            $this->replaceInFile('// Features::api(),', 'Features::api(),', config_path('authentication.php'));
        }
    ...
    protected function installLivewireStack()
    {
        ...
        // Service Providers...
        copy(__DIR__.'/../../stubs/app/Providers/AuthenticationServiceProvider.php', app_path('Providers/AuthenticationServiceProvider.php'));
        ServiceProvider::addProviderToBootstrapFile('App\Providers\AuthenticationServiceProvider');
...
EOF;
    ...
    protected function promptForMissingArgumentsUsing()
    {
        return [
            'stack' => fn () => select(
                label: 'Which Authentication stack would you like to install?',
                options: [
                    'livewire' => 'Livewire',
                ]
            ),
